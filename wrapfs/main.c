				...
				struct inode *inode)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		printk(KERN_INFO "before wrapfs_set_lower_inode i = %d\n", i);
		wrapfs_set_lower_inode_idx(inode, i,
				igrab(lower_dentry->d_inode));
		id1++;
		id2 = i;
	}
	printk(KERN_INFO "After for loop i = %d, idx = %d id2 = %d\n", i,
								id1, id2);
	...
	printk(KERN_INFO "lower_inode i = %d\n", i);
	...
	printk(KERN_INFO "wrapfs_fill_inode_successful\n");
}
							...
							char *options)
{
	...
	if (is_ldir_present && is_rdir_present) {
		...
		printk(KERN_ERR "wrapfs: no branches specified\n");
		err = -EINVAL;
		goto out_error;
		}
	goto out_return;
			...
			struct path *lower_path)
{
	...
	printk(KERN_INFO" Wrapfs_interpose\n");
	...
	UDBG;
	inode = wrapfs_iget(sb, iunique(sb, 1));
	UDBG;
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		goto out;
	...
	printk(KERN_INFO" Wrapfs_interpose success\n");
out_add:
	d_add(dentry, inode);
out:
	return err;
...
static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	if (!raw_data) {
		printk(KERN_ERR
			"wrapfs: read_super: missing data argument\n");
		err = -EINVAL;
		goto out;
	}
