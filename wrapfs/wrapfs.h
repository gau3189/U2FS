...
static inline void wrapfs_reset_lower_path_idx(const struct dentry *dent,
						int idx)
{
	/*spin_lock(&WRAPFS_D(dent)->lock);*/
...
static inline struct dentry *wrapfs_lower_dentry_idx(const struct dentry *dent,
							int index)
{
	BUG_ON(!dent || index < 0);
...
static inline struct vfsmount *wrapfs_lower_mnt_idx(const struct dentry *dent,
							int index)
{
	BUG_ON(!dent || index < 0);
					...
					int bstart, int bend, bool free_lower)
{
	...
	for (i = bstart; i <= bend; i++) {
		...
		lower_mnt = wrapfs_lower_mnt_idx(dentry, i);
		if (lower_mnt) {
...
static inline void path_put_lowers_all(struct dentry *dentry, bool free_lower)
{
	...
	path_put_lowers(dentry, 0, 1, free_lower);
}
			       ...
			       int bstart, int bend, bool free_lower)
{
	...
	for (bindex = bstart; bindex <= bend; bindex++) {
		lower_inode = wrapfs_lower_inode_idx(inode, bindex);
		if (lower_inode) {
	...
	if (free_lower) {
		kfree(WRAPFS_I(inode)->lower_inodes);
		WRAPFS_I(inode)->lower_inodes = NULL;
	}
}
...
static inline void iput_lowers_all(struct inode *inode, bool free_lower)
{
	...
	iput_lowers(inode, 0, 1, free_lower);
}
