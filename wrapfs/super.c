...
static void wrapfs_put_super(struct super_block *sb)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		s = wrapfs_lower_super_idx(sb, i);
		wrapfs_set_lower_super_idx(sb, i, NULL);
...
static int wrapfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	lower_dentry = wrapfs_lower_dentry(sb->s_root);
	lower_path.dentry = lower_dentry;
	lower_path.mnt = wrapfs_lower_mnt_idx(sb->s_root, 0);
...
{
	/* TODO : Locking semantic */
	struct super_block *lower_sb;
	int i = 0;

	for (i = 0; i <= 1; i++) {
		lower_sb = wrapfs_lower_super_idx(sb, i);
